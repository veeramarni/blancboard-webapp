...
jQuery.DrawEngine = (function() {
    var _modeSwitcher, _newProperties, _selectedObj, _wbElements, _ft;
    ...
    function _calculateOffset() {
        // left = _wbScroll.x - _drawOptions.offset.left;
        // top = _wbScroll.y - _drawOptions.offset.top;
    ...
    function _undoAction() {
        if (_drawOptions.paper.undocache.length > 0) {
    ...
    function _redoAction() {
        if (_drawOptions.paper.redocache.length > 0) {
    ...
    function _invokeDrawFunction(arg) {
        switch (arg.state) {
            case "onSend":
                ...
                break;
            case "onEnableUndo":
                if (undoActionConvert[arg.sendchanges.action]) {
            ...
            case "onBeforeDocCreate":
                ...
                break;
            case "onClearWhiteboard":
                if (typeof(_drawOptions.onClearWhiteboard) != 'undefined')
                    _drawOptions.onClearWhiteboard();
                break;
    ...
    function _switchToMode(mode, cursor) {
        ...
        // _drawOptions.whiteboard.css("cursor", cursor);
    };
    ...
    Raphael.mouseup(function() {
        ...
        if (_modeSwitcher.circleMode) {
            ...
            if (r == 0) {
                _invokeLogger({
                    "info": " Element was choosen but haven't drawn anything "
                });
                return;
            }
    ...
    function _removeElementAfterVerification(uuid, broadcast, isEnableUndo) {
        ...
        var hbr = _wbElements[uuid];
        if (hbr == null) {
            // not found ==> nothing to do
            _invokeLogger({
                "debug": "Element to be removed does not exist anymore in this Whiteboard"
            });
            return;
        }
    ...
    function _removeElement(helperBox, broadcast, isEnableUndo) {
        ...
        // TO DO
        // find when the paper is empty and unmark the paper as non
        // empty
        /*
         * if(!paperProperties.paper.bottom == undefined){
         * markEmptyPage(_self.currentPageId); }
         */
        _wbElements[eluuid] = null;
        delete _wbElements[eluuid];
    ...
    function _hideElements(ids) {
        ...
        if (typeof ids === 'string') {
            var hb = _wbElements[ids];
            _disableBindings(hb);
            hb && hb.element.hide();
        } else {
            for (var i = 0; i < ids.length; i++) {
                var hb = _wbElements[ids[i]];
                _disableBindings(hb);
                hb && hb.element.hide();
            };
        }
        ...
        if (typeof ids === 'string') {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                try {
                    ...
                    logError("Elements of the document appears to be in header but doesn't actually exist");
                }
    ...
    function _bringFrontElementAfterVerification(uuid, orderNo, broadcast, isEnableUndo) {
            ...
            var hbf = _wbElements[uuid];
            if (hbf == null) {
                // not found ==> nothing to do
                _invokeLogger({
                    "debug": "Element to be brought to front does not exist anymore in this Whiteboard"
                });
                return;
            }
    ...
    function _bringBackElementAfterVerification(uuid, orderNo, broadcast, isEnableUndo) {
            ...
            var hbb = _wbElements[uuid];
            if (hbb == null) {
                // not found ==> nothing to do
                _invokeLogger({
                    "debug": "Element to be brought to back does not exist anymore in this Whiteboard"
                });
                return;
            }
     ...
     * @public
     */
    function _clearWhiteboard(broadcast) {
        // send changes to server
        if (_drawOptions.isBroadcastable && broadcast) {
            var drawReturns = {};
            drawReturns.state = "onSend";
    ...
    function _clearDocElements(ids) {
            ...
            if (typeof ids === 'string') {
                ...
                if (hb !== undefined) {
                    hb.element.remove();
                    hb.remove();
                    _wbElements[ids] = null;
                    delete _wbElements[ids];
                }
            } else {
                for (var i = 0; i < ids.length; i++) {
                    var hb = _wbElements[ids[i]];
                    if (hb !== undefined) {
                        hb.element.remove();
                        hb.remove();
                        _wbElements[ids[i]] = null;
                        delete _wbElements[ids[i]];
                    }
                }
            }
        }
    ...
    function _clickEl(event) {
        ...
        if (_modeSwitcher.removeMode) {
            _removeElement(this, true, false);
            return true;
        }
        if (_modeSwitcher.bringFrontMode) {
            // forward order
    ...
    function _pasteIcon(x, y) {
        if (_drawOptions.whiteboard.iconEl.icon == null || _drawOptions.whiteboard.iconEl.icon === undefined) {
            logError("Clicking the whiteboard without selecting the icon.");
            return;
        }
    ...
    function _createElementAfterVerification(props, classType, orderNo, broadcast, isEnableUndo) {
            ...
            if (_wbElements[props.uuid] !== undefined) {
                return;
            };
    ...
    function _updateElement(sentProps, classType) {
        ...
        var hbu = _wbElements[sentProps.uuid];
        if (hbu == null) {
            // not found ==> nothing to do
            _invokeLogger({
                "debug": "Element to be updated does not exist anymore in this Whiteboard"
            });
            return;
        }
    ...
    function _trnsUnplug(flag) {
        ...
        else if (_selectedObj != null && _wbElements[_selectedObj.uuid]) {
            _selectedObj.hover(_hoverOverEl, _hoverOutEl);
        }
    ...
    return {
        ...
        clearWhiteboard: function(broadcast) {
            _clearWhiteboard(broadcast);
        },
        undoAction: function() {
            _undoAction();
        },
        ...
        getScale: function() {
            if (isNaN(scale)) {
                logError("Found the scale to NaN.Replacing with 1");
                return parseInt(1);
            }
            return scale;
        },
